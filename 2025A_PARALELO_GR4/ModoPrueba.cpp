...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // CALCULAR POSICIONES DE FOCOS (¡NUEVO! - debe hacerse después de actualizar posiciones)
        float angle1 = glm::radians(carRotation1);
        float angle2 = glm::radians(carRotation2);
        ...
        if (vistaPrimeraPersona1) {
            glm::vec3 arriba(0.0f, 1.2f, 0.0f);
            glm::vec3 adelante = glm::vec3(-sin(carRotation1), 0.0f, -cos(carRotation1)) * -2.0f;
            camPos1 = carPosition1 + arriba + adelante;
        ...
        if (vistaPrimeraPersona1) {
            cameraJugador1.Front = glm::vec3(sin(carRotation1), 0.0f, cos(carRotation1));
        }
        else {
        ...
        if (vistaPrimeraPersona2) {
            glm::vec3 arriba(0.0f, 1.2f, 0.0f);
            glm::vec3 adelante = glm::vec3(-sin(carRotation2), 0.0f, -cos(carRotation2)) * -2.0f;
            camPos2 = carPosition2 + arriba + adelante;
        ...
        if (vistaPrimeraPersona2) {
            cameraJugador2.Front = glm::vec3(sin(carRotation2), 0.0f, cos(carRotation2));
        }
        else {
        ...
        // ENVIAR DATOS DE LUCES AL SHADER (¡NUEVO! - para vista jugador 2)
        ourShader.setVec3("lightPos1Left", spotlightPos1Left);
        ourShader.setVec3("lightPos1Right", spotlightPos1Right);
        ourShader.setVec3("lightPos2Left", spotlightPos2Left);
        ourShader.setVec3("lightPos2Right", spotlightPos2Right);
        ...
        drtMatrix = glm::translate(drtMatrix, glm::vec3(0.0f, -10.0f, 0.0f));
        ...
        if (!car1OnTrack) {
            std::cout << "ˇAuto 1 fuera de pista (DRT)!" << std::endl;
            carSpeed1 *= 0.95f;
        }

        if (!car2OnTrack) {
            std::cout << "ˇAuto 2 fuera de pista (DRT)!" << std::endl;
            carSpeed2 *= 0.95f;
        }
